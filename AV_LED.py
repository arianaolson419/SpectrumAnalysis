...
def AudioVisualizer(strip, color, led_array, LED_COUNT):
	fps = 24
	q = LED_COUNT * 2 / 12
	for k in range(len(led_array)-1):
		frame = led_array[k]
		for i in frame:
			if i == 0:
				for j in range(frame[0]-1):
